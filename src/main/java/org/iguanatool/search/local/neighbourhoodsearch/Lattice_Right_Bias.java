...
public class Lattice_Right_Bias extends CachedNeighbourhoodSearch {
	...
	public Lattice_Right_Bias(RandomNumberGenerator rng) {
		super(rng);
	}
					...
					if (next.betterThan(current)) {
						...
						do {
							...
							if (getElement(middleRight, index) <= getElement(right, index)) {
								...
								if (!middleLeft.betterThan(middleRight)) {

...
public class Lattice_No_Bias extends CachedNeighbourhoodSearch {
	...
	public Lattice_No_Bias(RandomNumberGenerator rng) {
		super(rng);
	}
					...
					if (next.betterThan(current)) {
						...
						do {
							...
							if (getElement(middleRight, index) <= getElement(right, index)) {
								...
								if (middleRight.betterThan(middleLeft) || !middleRight.worseThan(middleLeft) && rng.nextBoolean()) {
